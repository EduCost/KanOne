...
class BoardViewModelTest {
    ...
    fun `GIVEN not null states, WHEN create card, THEN card repository is called`() {
        ...
        coEvery { cardRepository.createCard(any(), columnId) } returns Result.Success(1)
        ...
        runTest(testDispatcher) {
            viewModel.uiState.test {
                ...
                coVerify {
                    boardRepository.observeCompleteBoard(any())
                    cardRepository.createCard(any(), columnId)
                }
            }
        }
    }
    ...
    fun `GIVEN result error, WHEN create card, THEN snackbar is sent`() {
        ...
        coEvery { cardRepository.createCard(any(), columnId) } returns Result.Error(
